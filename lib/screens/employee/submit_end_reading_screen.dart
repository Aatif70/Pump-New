...
class SubmitEndReadingScreen extends StatefulWidget {
  final String? nozzleId;
  final String? shiftId;
  final String? fuelTankId;
  final String? petrolPumpId;

  const SubmitEndReadingScreen({
    Key? key,
    this.nozzleId,
    this.shiftId,
    this.fuelTankId,
  ...
  @override
  _SubmitEndReadingScreenState createState() => _SubmitEndReadingScreenState();
}

class _SubmitEndReadingScreenState extends State<SubmitEndReadingScreen> {
  ...
  Future<void> _initializeData() async {
    ...
    // Fetch previous readings if we have a nozzle ID
    if (_nozzleId != null) {
      await _fetchPreviousReadings();
    }
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      final res = await repo.submitEndNozzleReading(
        nozzleId: _nozzleId!,
        shiftId: _shiftId!,
      ...
      if (res.success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('End reading submitted successfully'),
            ...
            children: [
              Text(
                _selectedNozzleAssignment != null 
                    ? 'Record end reading for ${_selectedNozzleAssignment!.fuelType} - Nozzle ${_selectedNozzleAssignment!.nozzleNumber}'
                    : 'Capture the final meter reading to complete your shift',
                style: GoogleFonts.poppins(
            ...
            Container(
              ...
              child: TextFormField(
                ...
                decoration: InputDecoration(
                  ...
                  hintText: 'Enter the final reading on meter',
  ...
  Future<void> _getCurrentEmployeeId() async {
    try {
      ...
      if (empResp.success && empResp.data != null) {
        ...
        setState(() { _error = 'Failed to get current employee'; });
      }
    } catch (e) {
      print('Error getting current employee: $e');
      setState(() { _error = 'Error getting employee data'; });
    }
  }
  ...
  Future<void> _fetchNozzleAssignments() async {
    ...
    setState(() { _loadingNozzleAssignments = true; });
    
    try {
      final assignResp = await NozzleAssignmentRepository().getEmployeeNozzleAssignments(_employeeId!);
      
      if (assignResp.success && assignResp.data != null && assignResp.data!.isNotEmpty) {
        setState(() {
          _nozzleAssignments = assignResp.data!;
          ...
          // Otherwise select the first assignment by default
            ...
            // Update related fields based on selected assignment
        ...
        setState(() {
          _nozzleAssignments = [];
          _error = 'No nozzle assignments found';
        });
      }
    } catch (e) {
      print('Error fetching nozzle assignments: $e');
      setState(() { _error = 'Failed to fetch nozzle assignments'; });
    } finally {
      setState(() { _loadingNozzleAssignments = false; });
    }
  }
  
  Future<void> _fetchPreviousReadings() async {
    if (_nozzleId == null || _shiftId == null) return;
    
    setState(() { _fetchingPreviousReadings = true; });
    
    try {
      // Get previous start reading for this nozzle and shift
      final startReadingResp = await NozzleReadingRepository().getLatestReading(_nozzleId!, 'Start');

      
      // Get previous end reading for this nozzle
      final endReadingResp = await NozzleReadingRepository().getLatestReading(_nozzleId!, 'End');
      
      setState(() {
        _previousStartReading = startReadingResp.success ? startReadingResp.data : null;
        _previousEndReading = endReadingResp.success ? endReadingResp.data : null;
        
        // Prepopulate meter reading input with a suggestion based on previous start reading
        if (_previousStartReading != null && _meterReadingController.text.isEmpty) {
          _meterReadingController.text = (_previousStartReading!.meterReading).toStringAsFixed(2);
        }
      });
    } catch (e) {
  ...
  // Helper to get color for fuel type
  ...
  void _updateSelectedNozzleData() {
    if (_selectedNozzleAssignment != null) {
      ...
      // Fetch previous readings for the selected nozzle
      _fetchPreviousReadings();
